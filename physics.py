...
n1 = []
n2 = []
T_coeff, T_bar, a_T, b_T = 21, torch.tensor((1.5, 4.5, 15)), torch.tensor(
    (-0.2, -0.005, -0.00054)), torch.tensor((0, 0, 0))
S_bar, a_S, b_S = torch.tensor((20, 25)), torch.tensor(
    (-0.001, -0.0008)), torch.tensor((0, 0))
N1_bar, a_N1, b_N1 = torch.tensor((2, 5)), torch.tensor(
    (-0.002, -0.00054)), torch.tensor((0, 0))
N2_bar, a_N2, b_N2 = torch.tensor((4, 1)), torch.tensor(
    (-0.008, -0.0001)), torch.tensor((0, 0))
day_night = 2
...
def dynamics(ringo, color, time_step, RADIUS=10, COLL_DIST=10., MASS=1E-6, SPF=1/144, WIDTH=1600, HEIGHT=900):
    ...
    SOLAR = Solar_E(time_step, S_bar, a_S, b_S) * \
        (sin(time_step * day_night) + 1) * 0.5
    ...
    # Condition to prevent incorrect drag force(to preserve Conservation of Energy)  when slow particle velocity assumption is broken. We assume in such a case that the fluid exerts a linear drag if velocity increases corrosponding to the particles min. velocity threshold in the medium.
    v = v + (dv_drag * torch.where(torch.abs(v) < torch.abs(dv_drag), 0, 1)) - \
        (0.1 * v * torch.where(torch.abs(v) < torch.abs(dv_drag), 1, 0))
...
def main():
    ...
    cell_dynamics = torch.randint(30, (400, 4)).to(
        dtype=torch.float)  # [Cells[Alive],4]  x, y, vx, vy
